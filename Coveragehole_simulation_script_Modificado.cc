...
double rsrp3;
double rsrq3;
double previousrsrp1;
double previousrsrq1;
double previousrsrp2;
double previousrsrq2;
double previousrsrp3;
double previousrsrq3;
...
void 
ReportUeMeasurementsCallback (std::string context,
                        ...
                        uint16_t targetCellId)
{
  ...
  ho=true;
  Config::Disconnect ("/NodeList/0/DeviceList/*/LteUePhy/ReportUeMeasurements", 
                                                    ...
                                                    if(UeImsi==1)
                                                            {
                                                              ...
                                                              sstm << "filecomplete-" << targetCellId << "-" << RunValue;
                                                              AsciiTraceHelper asciiTraceHelper;
                                                              Ptr<OutputStreamWrapper> streamFileComplete = asciiTraceHelper.CreateFileStream(sstm.str());
                                                              *streamFileComplete->GetStream()<< RunValue << "\t" << Angle << "\t" << targetCellId << "\t"
                                                                           << Simulator::Now ().GetSeconds () << "\t" << sink1->GetTotalRx () << "\t"
                                                                           << rsrp1 << "\t" << rsrq1 << "\t"
                                                                           << rsrp2 << "\t" << rsrq2 << "\t"
                                                                           << rsrp3 << "\t" << rsrq3 << "\t"
                                                                           << previousrsrp1 << "\t" << previousrsrq1 << "\t"
                                                                           << previousrsrp2 << "\t" << previousrsrq2 << "\t"
                                                                           << previousrsrp3 << "\t" << previousrsrq3 << std::endl;
...
// connect custom trace sinks for RRC connection establishment and handover notification
  ...
  //Ycaro
  if(!ho)
  Config::Connect ("/NodeList/0/DeviceList/*/LteUePhy/ReportUeMeasurements", 
                   MakeCallback (&ReportUeMeasurementsCallback));
